...
public class Log {
  ...
  public Log(String focus, Long begin, Long end, String user, OrientGraph g) throws AccessDeniedException {
    ...
    if (AccessRights.getAccessRights(vUser, vTarget, g).value() < DataModel.Enums.AccessRights.READ.value()) {
      throw new AccessDeniedException("User " + user + " not authorized to access node " + focus);
    }
    this.focus = focus;
